...
async fn call_guest(mut store: &mut wasmtime::Store<()>, instance: &wasmtime::Instance, guest_data: &str) -> anyhow::Result<()> {
    ...
    let memory_address = alloc.call_async(&mut store, guest_data_bytes.len() as u32).await?;
    ...
    let guest_memory_offset = call_guest.call_async(&mut store, (memory_address, guest_data_bytes.len() as u32)).await?;
...
mod utils {
    ...
    pub fn read_string(offset: usize, mem: &wasmtime::Memory, store: &mut wasmtime::Store<()>) -> String {
